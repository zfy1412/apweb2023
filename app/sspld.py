...
def phe_sspld(pk, p0, p1, p2):
    ...
    b_squared = phe_protocol.PHEProtocol(p0[0] - p2[0]).phe_pow() + phe_protocol.PHEProtocol(p0[1] - p2[1]).phe_pow()
    c_squared = phe_protocol.PHEProtocol(p1[0] - p2[0]).phe_pow() + phe_protocol.PHEProtocol(p1[1] - p2[1]).phe_pow()
...
if __name__ == '__main__':
    ...
    p0 = [pk.encrypt(600), pk.encrypt(200)]
    p1 = [pk.encrypt(282), pk.encrypt(770)]
    p2 = [pk.encrypt(981), pk.encrypt(770)]
    p3 = [pk.encrypt(1), pk.encrypt(1)]
    p4 = [pk.encrypt(10), pk.encrypt(2)]
    s = time.time()

    a = phe_sspld(pk, p0, p1, p2)
    b = phe_sspld(pk, p0, p3, p4)
    c=pk.encrypt(10)
    d=pk.encrypt(-20)
    print(time.time() - s)
    print(sk.decrypt(a))
    print(sk.decrypt(b))
    print(phe_protocol.PHEProtocol(a).pin_boolmore(b))
